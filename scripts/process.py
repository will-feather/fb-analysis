from .db import BD
from .odds import main as od
from .matches import main as mt
import numpy as np
import pandas as pd
...
def inner_table_jogos(df_gp_jogos_todos, df_gp_jogos_casa_home, df_gp_jogos_casa_away):
    ...
    df_inner_merged_temp_jogos = pd.merge(pd.merge(df_gp_jogos_todos, df_gp_jogos_casa_home, on=["HomeTeam"]), df_gp_jogos_casa_away, on=["HomeTeam"]).round(2)
...
def calculate_probabilities(df_events_matches):
    ...
    df_events_matches['% Vitoria'] = round(((df_events_matches['Vitorias Home'] / 100) * 60) + ((df_events_matches['Vitorias Home Todos'] / 100) * 40), 2)
    df_events_matches['% Empate'] = round(((df_events_matches['Empates Home'] / 100) * 60) + ((df_events_matches['Empates Home Todos'] / 100) * 40), 2)
    df_events_matches['% Derrota'] = round(((df_events_matches['Derrotas Home'] / 100) * 60) + ((df_events_matches['Derrotas Home Todos'] / 100) * 40), 2)

    df_events_matches['% AM Sim'] = round(((((df_events_matches['AM Home']/100)*70)+((df_events_matches['AM Away']/100)*30))+((((df_events_matches['AM Home Todos']/100)*70)+((df_events_matches['AM Away Todos']/100)*30))))/2, 2)
    df_events_matches['% AM Não'] = 1 - df_events_matches['% AM Sim']

    df_events_matches['% Over 05 HT'] = round(((((df_events_matches['Over 05 HT Home']/100)*60)+((df_events_matches['Over 05 HT Away']/100)*40))+((((df_events_matches['Over 05 HT Home Todos']/100)*60)+((df_events_matches['Over 05 HT Away Todos']/100)*40))))/2, 2)
    df_events_matches['% Over 15 HT'] = round(((((df_events_matches['Over 15 HT Home']/100)*60)+((df_events_matches['Over 15 HT Away']/100)*40))+((((df_events_matches['Over 15 HT Home Todos']/100)*60)+((df_events_matches['Over 15 HT Away Todos']/100)*40))))/2, 2)
    df_events_matches['% Over 25 HT'] = round(((((df_events_matches['Over 25 HT Home']/100)*60)+((df_events_matches['Over 25 HT Away']/100)*40))+((((df_events_matches['Over 25 HT Home Todos']/100)*60)+((df_events_matches['Over 25 HT Away Todos']/100)*40))))/2, 2)

    df_events_matches['% Over 05'] = round(((((df_events_matches['Over 05 Home']/100)*60)+((df_events_matches['Over 05 Away']/100)*40))+((((df_events_matches['Over 05 Home Todos']/100)*60)+((df_events_matches['Over 05 Away Todos']/100)*40))))/2, 2)
    df_events_matches['% Over 15'] = round(((((df_events_matches['Over 15 Home']/100)*60)+((df_events_matches['Over 15 Away']/100)*40))+((((df_events_matches['Over 15 Home Todos']/100)*60)+((df_events_matches['Over 15 Away Todos']/100)*40))))/2, 2)
    df_events_matches['% Over 25'] = round(((((df_events_matches['Over 25 Home']/100)*60)+((df_events_matches['Over 25 Away']/100)*40))+((((df_events_matches['Over 25 Home Todos']/100)*60)+((df_events_matches['Over 25 Away Todos']/100)*40))))/2, 2)
    df_events_matches['% Over 35'] = round(((((df_events_matches['Over 35 Home']/100)*60)+((df_events_matches['Over 35 Away']/100)*40))+((((df_events_matches['Over 35 Home Todos']/100)*60)+((df_events_matches['Over 35 Away Todos']/100)*40))))/2, 2)
    df_events_matches['% Over 45'] = round(((((df_events_matches['Over 45 Home']/100)*60)+((df_events_matches['Over 45 Away']/100)*40))+((((df_events_matches['Over 45 Home Todos']/100)*60)+((df_events_matches['Over 45 Away Todos']/100)*40))))/2, 2)
    df_events_matches['% Over 55'] = round(((((df_events_matches['Over 55 Home']/100)*60)+((df_events_matches['Over 55 Away']/100)*40))+((((df_events_matches['Over 55 Home Todos']/100)*60)+((df_events_matches['Over 55 Away Todos']/100)*40))))/2, 2)
    df_events_matches['% Over 65'] = round(((((df_events_matches['Over 65 Home']/100)*60)+((df_events_matches['Over 65 Away']/100)*40))+((((df_events_matches['Over 65 Home Todos']/100)*60)+((df_events_matches['Over 65 Away Todos']/100)*40))))/2, 2)
    ...
    df_events_matches['% Vitoria Away'] = round(((df_events_matches['Vitorias Away'] / 100) * 60) + ((df_events_matches['Vitorias Away Todos'] / 100) * 40), 2)
    df_events_matches['% Empate Away'] = round(((df_events_matches['Empates Away'] / 100) * 60) + ((df_events_matches['Empates Away Todos'] / 100) * 40), 2)
    df_events_matches['% Derrota Away'] = round(((df_events_matches['Derrotas Away'] / 100) * 60) + ((df_events_matches['Derrotas Away Todos'] / 100) * 40), 2)

    df_events_matches['% Media Vitoria'] = (df_events_matches['% Vitoria']+(1-df_events_matches['% Vitoria Away']))/2
    df_events_matches['% Media Empate'] = (df_events_matches['% Empate']+(1-df_events_matches['% Empate Away']))/2
    df_events_matches['% Media Derrota'] = (df_events_matches['% Derrota']+(1-df_events_matches['% Derrota Away']))/2
    df_events_matches['% Media Total'] = df_events_matches['% Media Vitoria'] + df_events_matches['% Media Empate'] + df_events_matches['% Media Derrota'] 
...
def prepara_odds_events(df_odds_eventos):
    ...
    df_odds_eventos['Div Vitoria'] = round(100/(df_odds_eventos['% Media Vitoria'] * 100), 2)
    df_odds_eventos['Div Empate'] = round(100/(df_odds_eventos['% Media Empate'] * 100), 2)
    df_odds_eventos['Div Derrota'] = round(100/(df_odds_eventos['% Media Derrota'] * 100), 2)

    df_odds_eventos['Div AM Sim'] = round(100/(df_odds_eventos['% AM Sim'] * 100), 2)
    df_odds_eventos['Div AM Não'] = round(100/(df_odds_eventos['% AM Não'] * 100), 2)

    df_odds_eventos['Div Over 05 HT'] = round(100/(df_odds_eventos['% Over 05 HT'] * 100), 2)
    df_odds_eventos['Div Over 15 HT'] = round(100/(df_odds_eventos['% Over 15 HT'] * 100), 2)
    df_odds_eventos['Div Over 25 HT'] = round(100/(df_odds_eventos['% Over 25 HT'] * 100), 2)

    df_odds_eventos['Div Over 05'] = round(100/(df_odds_eventos['% Over 05'] * 100), 2)
    df_odds_eventos['Div Over 15'] = round(100/(df_odds_eventos['% Over 15'] * 100), 2)
    df_odds_eventos['Div Over 25'] = round(100/(df_odds_eventos['% Over 25'] * 100), 2)
    df_odds_eventos['Div Over 35'] = round(100/(df_odds_eventos['% Over 35'] * 100), 2)
    df_odds_eventos['Div Over 45'] = round(100/(df_odds_eventos['% Over 45'] * 100), 2)
    df_odds_eventos['Div Over 55'] = round(100/(df_odds_eventos['% Over 55'] * 100), 2)
    df_odds_eventos['Div Over 65'] = round(100/(df_odds_eventos['% Over 65'] * 100), 2)
...
def cross_table_events(df_odds_eventos):
    ...
    df_clear_odds_eventos = df_odds_eventos.drop(columns=['% Media Vitoria', '% Media Empate', '% Media Derrota', '% AM Sim', '% AM Não', '% Over 05 HT', '% Over 15 HT', 
            '% Over 25 HT', '% Over 05', '% Over 15', '% Over 25', '% Over 35', '% Over 45', '% Over 55', '% Over 65'])
    ...
    'Div AM Não': 'AM Não', 'Div Over 05 HT': 'Over 05 HT', 'Div Over 15 HT': 'Over 15 HT', 'Div Over 25 HT': 'Over 25 HT', 'Div Over 05': 'Over 05', 'Div Over 15': 'Over 15', 
    'Div Over 25': 'Over 25', 'Div Over 35': 'Over 35', 'Div Over 45': 'Over 45', 'Div Over 55': 'Over 55', 'Div Over 65': 'Over 65'}, inplace=True)
    ...
    entrada_values = ['C1', 'E2', 'F3', 'S4', 'N5', 'OH' + df_cross_table_eventos['Mercado Divisão'].str[-5:-3], ]
...
def cross_table_probabilities(df_odds_probabilidades):
    ...
    df_odds_probabilidades.rename(columns={'% Media Vitoria': 'Casa Vitoria', '% Media Empate': 'Casa Empate', '% Media Derrota': 'Casa Derrota', '% AM Sim': 'AM Sim', 
    '% AM Não': 'AM Não', '% Over 05 HT': 'Over 05 HT', '% Over 15 HT': 'Over 15 HT', '% Over 25 HT': 'Over 25 HT', '% Over 05': 'Over 05', '% Over 15': 'Over 15', 
    '% Over 25': 'Over 25', '% Over 35': 'Over 35', '% Over 45': 'Over 45', '% Over 55': 'Over 55', '% Over 65': 'Over 65'}, inplace=True)
    ...
    entrada_values = ['C1', 'E2', 'F3', 'S4', 'N5', 'OH' + df_cross_table_probabilidades['Mercado Prob'].str[-5:-3], ]
...
def create_odds_ids(df_odds):
    ...
    entrada_values = ['C1', 'F3', 'E2', 'S4', 'N5', 'OH' + df_odds['Mercado'].str[-8:-5].str.replace('.', ''), ]
...
def odds_prob_events_results(df_odds, df_cross_table_eventos, df_cross_table_probabilidades):
    ...
    inner_merged_odds_prob_ev['EV'] = round(inner_merged_odds_prob_ev['Odd']/inner_merged_odds_prob_ev['Divisão']-1, 2)
    inner_merged_odds_prob_ev['EV'] = inner_merged_odds_prob_ev['EV'].astype(float).map("{:.1%}".format)
